...
# VAE的中间层，在均值和方差的基础上加上从正态分布采样的噪声生成latent vector
def sampling(args):
    #import tensorflow as tf
...
# 添加中间层，对均值和方差加入噪声；Lambda将任意表达式封装为Layer对象
z = Lambda(sampling, output_shape=(latent_dim, ))([z_mean_encoded, z_log_var_encoded])

# dropout，用来防止过拟合,在Decoder的输入和第一层隐藏层之间加入，每轮迭代时每五个输入值就会被随机抛弃一个
...
hist = vae.fit(np.array(rnaseq_train_df),
               ...
               validation_data=(np.array(rnaseq_test_df), None),  # 评估损失
               callbacks=[WarmUpCallback(beta, kappa), # 一系列可以在训练时使用的回调函数
...
# 把第一个文件的全部数据通过encoder进行压缩，生成latent。
...
###################################
# 把第二个文件的数据通过encoder进行压缩，生成latent。
