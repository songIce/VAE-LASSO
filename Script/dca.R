          ...
          smooth = FALSE, loess.span = 0.1, cmprsk = FALSE) 
{
  ...
  N = dim(data)[1]
  if (cmprsk == FALSE) {
    km.cuminc = survival::survfit(Surv(data.matrix(data[ttoutcome]), 
  ...
  for (m in 1:pred.n) {
    ...
    for (t in 1:length(nb$threshold)) {
      ...
      if (px == 0) {
        ...
        nb[t, predictors[m]] = pdgivenx * px - (1 - 
                                                  pdgivenx) * px * nb$threshold[t]/(1 - nb$threshold[t]) - 
          harm[m]
      }
    }
  ...
  if (graph == TRUE) {
    if (intervention == TRUE) {
      ...
      plot(x = nb$threshold, y = nb$all, type = "n", xlim = c(xstart, 
                                                              xstop), ylim = c(ymin, ymax), xlab = "Threshold probability", 
           ylab = paste("Net reduction in interventions per", 
      ...
      graphics::plot(x = nb$threshold, y = nb$all, type = "l",  cex.axis = 1.4, cex.lab=1.6,
                     col = 8, lwd = 4, xlim = c(xstart, xstop), ylim = c(ymin, 
                                                                         ymax), xlab = "Threshold probability", ylab = "Net benefit")
    ...
    graphics::legend("topright", legendlabel, cex = 1.1,
                     col = legendcolor, lwd = legendwidth, lty = legendpattern)
  }
  results = list()
  results$N = N
  results$predictors = data.frame(cbind(predictors, harm, 
                                        probability))
